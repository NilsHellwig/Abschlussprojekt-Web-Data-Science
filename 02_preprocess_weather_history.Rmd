...
for (html_path in html_documents) {
  ...
  for (weather_data_string in extracted_strings) {
    ...
    # somethimes, there are objects listed without actual weather data for a specific point in time. 
...
```{r}
# store the date and time in a seperate column
weather_data$ds <- as.POSIXlt(weather_data$ds, format = "%A, %d. %B %Y, %H:%M —")
...
```
```{r}
names(weather_data)[names(weather_data) == 'desc'] <- 'description'
