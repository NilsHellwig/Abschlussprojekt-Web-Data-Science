...
train_model <- function () {
  ctrl <- trainControl(verboseIter = TRUE, method="none")#, method = "cv", number = 5)
...
evaluate_model <- function () {
  ...
  row_index <- which(year_month_pairs$departure_scheduled_month_local == 10 &
                   year_month_pairs$departure_scheduled_year_local == 2017)
  
  new_row <- c(cf$overall, year_start_test = year_start_test, month_start_test=month_start_test) 
  print(new_row)
  results_df <- rbind(results_df, new_row)
  colnames(results_df) <- c(names(cf$overall), "year_start_test", "month_start_test")
  
  ##########
  col_index <- feature_importance$importance %>% 
