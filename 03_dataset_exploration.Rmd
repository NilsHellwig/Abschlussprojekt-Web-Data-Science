...
top_weather_delay
```
We can also look at how temperature affects delays

```{r}
ggplot(delay_departure_data, aes(x = as.factor(temperature_celsius), y = delay_departure_min)) +
  geom_point() +
  labs(x = "Temperature (Celsius)", y = "Departure Delay (Minutes)") +
  ggtitle("Influence of temperature on delays")
```

```{r}
filter(departure_data, temperature_celsius < 0)
```
