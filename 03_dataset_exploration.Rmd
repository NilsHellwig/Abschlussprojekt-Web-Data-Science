...
departure_data$weather_description <- sapply(departure_data$departure_scheduled_datetime_local, get_weather_data)
```
...
delay_departure_data <- delay_departure_data[!(format(delay_departure_data$departure_scheduled_datetime_local, "%Y-%m") %in% c("2017-09", "2023-05")), ]
```
...
```{r}
ggplot(delay_departure_data, aes(x = departure_scheduled_month_local, y = delay_departure_min)) +
  geom_boxplot() +
...
ggplot(delay_departure_data %>% filter(departure_scheduled_year_local %in% years_no_pandemic), aes(x = departure_scheduled_month_local, y = delay_departure_min)) +
  geom_boxplot() +
...
```{r}
ggplot(delay_departure_data, aes(x = departure_scheduled_time_local_hour_local, y = delay_departure_min)) +
  geom_boxplot() +
