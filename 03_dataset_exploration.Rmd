...
# For better data handling, we´ll create separate columns for the month, year and hour of the planned departure.
departure_data$departure_scheduled_month_year <- format(departure_data$date_departure, "%Y-%m")
departure_data$departure_scheduled_month <- format(departure_data$date_departure, "%m")
departure_data$departure_scheduled_year <- format(departure_data$date_departure, "%Y")
...
departure_data$departure_scheduled_weekday <- factor(weekdays(departure_data$date_departure), levels = weekday_order)
...
departure_data$departure_scheduled_time_local <- as.POSIXct(paste(departure_data$date_departure, departure_data$departure_scheduled_time_local), format = "%Y-%m-%d %H:%M")
```

```{r}
get_weather_data <- function(departure_time) {
  if (format(departure_time, format = "%Y-%m-%d 06:00:00 CEST") > departure_time) {
    weather_span <- format(departure_time, format = "%Y-%m-%d 00:00:00 CEST")
  } else if (format(departure_time, format = "%Y-%m-%d 12:00:00") > departure_time) {
    weather_span <- format(departure_time, format = "%Y-%m-%d 06:00:00 CEST")
  } else if (format(departure_time, format = "%Y-%m-%d 18:00:00") > departure_time) {
    weather_span <- format(departure_time, format = "%Y-%m-%d 12:00:00 CEST")
  } else if (format(departure_time, format = "%Y-%m-%d 24:00:00") > departure_time) {
    weather_span <- format(departure_time, format = "%Y-%m-%d 18:00:00 CEST")
  } 
  nearest_row <- weather_data[weather_data$six_hours_starting_from_datatime == weather_span,]$description[1]
  return(nearest_row)
}
departure_data$weather_description <- sapply(departure_data$departure_scheduled_time_local, get_weather_data)
```
...
flight_status_count <- departure_data %>%
  group_by(departure_scheduled_year, departure_scheduled_month, flight_status) %>%
  summarise(count = n())

ggplot(flight_status_count, aes(x = departure_scheduled_month, y = count, fill = flight_status)) +
  geom_bar(stat = "identity", position = "stack") +
  facet_wrap(~departure_scheduled_year, ncol = 1) +
  xlab("Month") +
  ylab("# Flights") +
...
flight_count_departure <- departure_data[!(format(departure_data$date_departure, "%Y-%m") %in% c("2017-09", "2023-05")), ] %>%
  group_by(departure_scheduled_month_year) %>%
  summarise(count = n())

ggplot(flight_count_departure, aes(x = as.Date(paste(departure_scheduled_month_year, "01", sep = "-")), y = count)) +
  geom_line() +
...
ggplot(average_delay_destination_departures, aes(x = as.Date(paste(departure_scheduled_month_year, "01", sep = "-")), y = avg_delay)) +
  geom_bar(stat = "identity", fill = "blue") +
  scale_x_date(date_labels = "%b %Y", date_breaks = "1 year") +
...
```{r}
ggplot(delay_departure_data, aes(x = departure_scheduled_month, y = delay_departure_min)) +
  geom_boxplot() +
...
ggplot(delay_departure_data %>% filter(departure_scheduled_year %in% years_no_pandemic), aes(x = departure_scheduled_month, y = delay_departure_min)) +
  geom_boxplot() +
...
```{r}
ggplot(delay_departure_data, aes(x = departure_scheduled_weekday, y = delay_departure_min)) +
  geom_boxplot() +
