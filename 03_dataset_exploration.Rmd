...
weather_data$six_hours_starting_from <- as.POSIXct(weather_data$six_hours_starting_from, format = "%H:%M")

match_weather_icon <- function(departure_time) {
  interval_start <- as.POSIXct(format(departure_time, "%Y-%m-%d %H:00"))
  interval_end <- interval_start + hours(6)
  print(interval_start)
  print(interval_end)
  print(departure_time)
  
  matching_row <- weather_data$date == departure_time$date_departure &
                   weather_data$six_hours_starting_from >= interval_start &
                   weather_data$six_hours_starting_from < interval_end
  
  if (any(matching_row)) {
    return(weather_data$weather_icon[matching_row])
  } else {
    return(NA)
  }
}
...
departure_data$weather_icon <- sapply(departure_data$departure_datetime, match_weather_icon)
```
