...
# For easy processing, I create separate columns for the month, year and hour of the planned departure.
...
departure_data$departure_scheduled_time_local <- as.POSIXct(departure_data$departure_scheduled_time_local, format = "%H:%M")
departure_data$departure_datetime <- as.POSIXct(paste(departure_data$date_departure, format(departure_data$departure_scheduled_time_local, "%H:%M")), format = "%Y-%m-%d %H:%M")
```
...
get_weather_data <- function(departure_time) {
  ...
  nearest_row <- weather_data[weather_data$datetime == weather_span,]$description[1]
  return(nearest_row)
}
