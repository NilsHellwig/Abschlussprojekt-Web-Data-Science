...
In this notebook, the aim is to predict whether a particular flight has (a) little/no delay or (b) long/very long delay, i.e. a binary classification. Previously, I tried to predict the delay of flights departing between 05/2022 and 05/2023 using the flight data from 10/2017 to 5/2022, also in a binary way. I was able to achieve a very poor accuracy (about 50%). 
...
There are extreme outliers in the data set. I have no explanation for this, probably some technical problems...
...
In order to remove the outliers, the code calculates the z-scores for the "delay_departure_min" column in the dataset using the scale() function. The z-score measures how many standard deviations an observation is away from the mean.
...
```{r}
write.csv(results_df, paste("results_", model_name_method, ".csv"))
head(results_df)
```
