...
train_df$delay_departure_min_category <- cut(train_df$delay_departure_min, 
                                             ...
                                             labels = c("on time or earlier", "short delay", "medium delay", "long delay"),
                                             include.lowest = TRUE)
...
filtered_train_df <- train_df[sample(nrow(train_df), 2000), 
                              -(which(names(train_df) %in% c("departure_scheduled_datetime_local", "delay_departure_min")))]


model <- train(delay_departure_min_category ~ departure_scheduled_weekday_local +
         departure_scheduled_hour_local +
         departure_scheduled_month_local +
         departure_scheduled_year_local +
         temperature_celsius,
...
```{r}
filtered_test_df <- test_df[-(which(names(test_df) %in% c("departure_scheduled_datetime_local", "delay_departure_min")))]
predictions <- predict(model, newdata = filtered_test_df)

# Evaluate the model performance
confusionMatrix(predictions, test_df$delay_departure_min_category)
```
